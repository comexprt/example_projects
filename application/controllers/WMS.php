...
class WMS extends CI_Controller {
		   ...
		   if($result)
		   {
			 ...
			 foreach($result as $row)
			 {
			   ...
			   }elseif($sess_array['Access_level']=="3")
			   {
				...
				$this->model_get->addlogs($newRow2);
	...
	public function emp_logout(){
		if ($this->session->userdata('logged_ADMIN') || $this->session->userdata('logged_PO')|| $this->session->userdata('logged_EU')){
		...
		if($this->session->userdata('logged_ADMIN')){
			...
			$Username = $session_data['Username'];
		}elseif($this->session->userdata('logged_EU')){
		...
		if($this->session->userdata('logged_PO')){
          $WSid = $this->input->post('WSid');
		   $Spare_Name = $this->input->post('Spare_Name');
			$new_Quantity_onHand = $this->input->post('Quantity_onHand');
			$new_Quantity_onOrder = $this->input->post('Quantity_onOrder');
			$new_ReOrdering_Pt = $this->input->post('ReOrdering_Pt');
			$new_replenishment = $this->input->post('replenishment');
			
			/*
			If (($new_Quantity_onHand <= $new_ReOrdering_Pt) AND ($new_Quantity_onOrder <=0)){
				$update_Quantity_onOrder= 
			...
			*/
			$newRow=array(
				...
				"replenishment" => $this->input->post('replenishment')
				 ); 
	...
	public function do_upload(){
		...
		if($this->session->userdata('logged_PO')){
          ...
          $Spare_Name = $this->input->post('Spare_Name');
		   $config =  array(
                  ...
                  'overwrite'       => TRUE,
                  'max_size'        => "30480000",  // Can be set to particular file size
                  'max_height'      => "1400",
                  'max_width'       => "2048"  
                );   
		...
		$this->load->library('upload', $config);
				...
				if($this->upload->do_upload())
				{
					...
					"Description" => $this->input->post('Description'),
					"Quantity_onHand" => 0,
					"Quantity_onOrder" =>$this->input->post('replenishment'),
					"ReOrdering_Pt" => $this->input->post('ReOrdering_Pt'),
					"Unit_Of_Measurement" => $this->input->post('Unit_Of_Measurement'),
					"Delivery_Price" => $this->input->post('Delivery_Price'),
					"replenishment" => $this->input->post('replenishment'),
				...
				else
				{
				   ...
				   $message = "$Spare_Name Added but Error uploading image.. Max Dimension 2048 X 1400 and valid extension jpg|png|jpeg|gif";
	...
	public function UpdateSpareToRequest(){
		...
		if ($action == "EditSpareToDraft"){
            ...
            $this->session->set_flashdata('message',"$message");
			$this->model_get->Update_Spare_Request_Details($WSid,$WRId,$newRow);
	...
	public function editrequest(){
		...
		if($this->session->userdata('logged_EU') || $this->session->userdata('logged_PO')){
	      ...
	      $newRow=array(
			...
			$this->model_get->editrequest($WRId,$newRow);
	...
	public function add_spare(){
		...
		if($this->session->userdata('logged_EU')){
		 ...
		 $counter=0;
		  $result_duplicate = $this->model_get->Spare_Request_Details_sid($WSid,$WRId);
	...
	public function delete_spare_request(){
		...
		if($this->session->userdata('logged_EU')){
          $WRId = $this->input->post('WRId');
			...
			$this->model_get->Delete_Spare_Request($WRId);
	...
	public function delete_spare(){
		...
		if($this->session->userdata('logged_EU')){
          ...
          $WSid = $this->input->post('WSid');
			...
			$this->model_get->Delete_Spare_Request_Details2($WSid);
...
public function Spare_Purchases_Info(){
		...
		if($this->session->userdata('logged_PO')){
			...
			$data['Status'] = "$Status";
			$data['Category'] = $this->model_get->getCategory();
				$data['SpareInfo'] = $this->model_get->Spare_Purchases_Details($Date);
				//$data['DraftInfo'] = $this->model_get->getSpecificRequest($WRId);
	...
	public function PR_Approval_Info(){
		...
		if($this->session->userdata('logged_PR')){
			...
			$data['Category'] = $this->model_get->getCategory();
				$data['SpareInfo'] = $this->model_get->Spare_Purchases_Details($Date);
				//$data['draft'] = $this->model_get->getallSparePurchase();
				$data['draft3'] = $this->model_get->getPR_Status($Date);
	...
	public function new_user(){
		...
		if ($position == "Administrator"){
		 ...
		 $newRow2=array(
			...
			$newRow4=array(
				"Status" => "User: ".$Lname.", ".$this->input->post('Fname')." ".$this->input->post('Mname').". - ID :".$this->input->post('DceNo')." Has been Added",
				"Action" => "Adding User - ".$position1,
				"ip" => $this->input->ip_address(),
				"username" => $Username
				);
				
			$this->model_get->addlogs($newRow4);
	...
	public function DeleteUser(){
		...
		if($this->session->userdata('logged_ADMIN')){
		 ...
		 if ($action == "delete-user"){
			...
			$newRow4=array(
				"Status" => "User : ".$name." - ID : ".$DceNo." Has been Deleted",
				"Action" => "Deleting User - ".$position,
				"ip" => $this->input->ip_address(),
				"username" => $Username
				);
		...
		if ($position == "Property Officer"){
		 ...
		 $newRow2=array(
			...
			$this->model_get->editUser($DceNoHidden,$newRow2);
			 ...
			 $newRow4=array(
				"Status" => $Lname.", ".$this->input->post('Fname')." ".$this->input->post('Mname').". User Has been Modified",
				"Action" => "Modifying User - ".$position1,
				"ip" => $this->input->ip_address(),
				"username" => $Username
				);
	...
	public function logs(){
		if($this->session->userdata('logged_ADMIN')){
		  ...
		  $this->session->keep_flashdata('message');
			...
			$data['Position'] = "$Position";
			//print_r ($data);
			$data['logs'] = $this->model_get->getAllLogs();
	...
	public function new_supplier(){
		...
		if($this->session->userdata('logged_ADMIN')){
		 ...
		 $newRow2=array(
			...
			$newRow4=array(
				"Status" => "Supplier : ".$this->input->post('Supplier_Name').". - ID : ".$this->input->post('SDceNo')." Has been Added",
				"Action" => "Adding Supplier",
				"ip" => $this->input->ip_address(),
				"username" => $Username
				);
	...
	public function DeleteSupplier(){
		...
		if($this->session->userdata('logged_ADMIN')){
		 ...
		 if ($action == "delete-supplier"){
			...
			$newRow4=array(
				"Status" => "Supplier : ".$name.". - ID : ".$SDceNo." Has been Deleted",
				"Action" => "Deleting Supplier",
				"ip" => $this->input->ip_address(),
				"username" => $Username
				);
	...
	public function editSupplier(){
		...
		if($this->session->userdata('logged_ADMIN')){
		 ...
		 $newRow2=array(
			...
			$newRow4=array(
				"Status" => "Supplier : ".$this->input->post('Supplier_Name').". - ID : ".$this->input->post('SDceNo')." Has been Modified",
				"Action" => "Modifying Supplier",
				"ip" => $this->input->ip_address(),
				"username" => $Username
				);
	...
	public function editFixedValue(){
		...
		if($this->session->userdata('logged_ADMIN')){
		 ...
		 $newRow2=array(
			...
			$newRow4=array(
				"Status" => $this->input->post('Name').": ".$this->input->post('Value')." Has been Modified",
				"Action" => "Modifying Fixed Value",
				"ip" => $this->input->ip_address(),
				"username" => $Username
				);
